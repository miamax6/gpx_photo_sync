#!/usr/bin/env python3
"""
GPX → Photos (RAW and JPEG) synchronization script
...
def setup_logging(gpx_file):
    ...
    class SafeFileHandler(logging.FileHandler):
        """File handler with safe flush handling"""
        def flush(self):
            try:
                super().flush()
            except OSError:
...
def parse_gpx(gpx_file):
    ...
    try:
        ...
        print(f"   🔍 Found {len(trkpts)} raw trackpoints")
        ...
        for idx, trkpt in enumerate(trkpts):
            ...
            if idx == 0:
                print(f"   🔍 First trkpt contains: {[child.tag for child in trkpt]}")
            ...
            if time_elem is None or not time_elem.text:
                if idx < 5:  # Only show first 5 to avoid spam
                    print(f"   ⚠️  Skipping point without timestamp: {lat}, {lon}")
                continue
            ...
            try:
                ...
                if idx < 5:
                    print(f"   ⚠️  Error parsing date '{time_elem.text}': {e}")
                continue
        ...
        print(f"📍 {len(points)} valid GPX points loaded from {gpx_file}")
        if points:
            print(f"   📅 Period: {points[0].time} → {points[-1].time}")
        return points
...
def update_photo_metadata(image_path, gpx_point, backup=True, dry_run=False):
    ...
    try:
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            # Log warnings more discreetly
            for warning in w:
                if "Strip 0 is outside of the data area" in str(warning.message):
                    logging.debug(f"Expected warning while processing NEF: {warning.message}")
                else:
                    logging.warning(f"Warning while processing image: {warning.message}")
        ...
        print(f"      ✓ GPS and IPTC updated")
        logging.info(f"Successfully updated GPS and IPTC data for {os.path.basename(image_path)}")
        return True
        ...
        if tmpdir and os.path.exists(tmpdir):
            try:
                ...
                logging.warning(f"Could not clean up temporary directory {tmpdir}: {str(e)}")
...
def process_photos(photos_folder, gpx_points, backup=True, dry_run=False):
    ...
    print(f"\n🔍 Searching for photos in: {photos_folder}")
    ...
    for photo_path in photo_files:
        ...
        if not photo_time:
            print(f"   ⚠️  No EXIF date found - SKIPPED")
        ...
        if closest_point is None:
            print(f"   ⏱️  Time difference > 1h ({time_diff/60:.1f} min) - SKIPPED")
            stats['skipped_too_far'] += 1
            continue
        
        # Show info
        print(f"   ✓ Match found (difference: {time_diff:.0f}s)")
...
Notes:
  ...
  - IPTC tags written: City, Province/State, Country Name, Country Code
    ...
    if not os.path.exists(args.gpx_file):
        print(f"❌ Error: GPX file '{args.gpx_file}' does not exist")
        sys.exit(1)
    
    if not os.path.exists(args.photos_folder):
        print(f"❌ Error: Folder '{args.photos_folder}' does not exist")
        sys.exit(1)
    ...
    if not gpx_points:
        print("❌ No GPX points found in file")
        sys.exit(1)
    ...
    print(f"✅ Synchronized:       {stats['synced']}")
    print(f"⏱️  Skipped (> 1h):     {stats['skipped_too_far']}")
    print(f"⚠️  Skipped (no date): {stats['skipped_no_date']}")
