...
generate_gpx() {
    ...
    if [ "$gpx_output" = "$photo_source" ]; then
        python3 photo_gps_to_gpx_fr.py "$photo_source" $anonymize_flag
    else
        python3 photo_gps_to_gpx_fr.py "$photo_source" "$gpx_output" $anonymize_flag
    fi
...
sync_gpx() {
    ...
    python3 sync_gpx_to_photos_fr.py "$gpx_file" "$nef_folder" $backup_flag $dryrun_flag
...
pipeline_complet() {
    ...
    python3 sync_gpx_to_photos_fr.py "$generated_gpx" "$nef_folder" $backup_flag $dryrun_flag
